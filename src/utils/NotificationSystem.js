...
class NotificationSystem {
     ...
     */
    sendNotification({ title, body, icon, onClick, id, actions, requireInteraction = false }) {
    ...
    scheduleNotification(options, scheduledTime) {
      ...
      const scheduleTime = scheduledTime instanceof Date 
        ? scheduledTime.getTime() 
        : scheduledTime;
      
      const delay = Math.max(0, scheduleTime - now);
      const notificationId = options.id || `notification_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
