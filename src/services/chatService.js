...
const generateResponse = async (message, file = null, documents = []) => {
  ...
  if (lowerMsg.includes('medication') || lowerMsg.includes('medicine') || lowerMsg.includes('prescription')) {
    return {
      content: "I don't see any medications in your records yet. You can upload your prescription documents or add medications manually in the Medications section.",
      timestamp: new Date().toISOString(),
      referencedDocuments: []
    };
  } 
  
  if (lowerMsg.includes('appointment') || lowerMsg.includes('doctor') || lowerMsg.includes('visit')) {
    return {
      content: "Your next appointment is not scheduled yet. Would you like to use the appointment scheduler to find a healthcare provider?",
      timestamp: new Date().toISOString(),
      referencedDocuments: []
    };
  }
  
  if (lowerMsg.includes('health') || lowerMsg.includes('condition') || lowerMsg.includes('symptom')) {
    return {
      content: "Based on the documents you've uploaded, I don't have enough information to provide health insights yet. Please upload your medical records through the Documents section.",
      timestamp: new Date().toISOString(),
      referencedDocuments: []
    };
  }
  
  if (lowerMsg.includes('document') || lowerMsg.includes('record') || lowerMsg.includes('report')) {
    if (documents.length > 0) {
      ...
      return {
        content: "You haven't uploaded any documents yet. Would you like me to guide you through the document upload process?",
        timestamp: new Date().toISOString(),
        referencedDocuments: []
      };
    }
  ...
  if (lowerMsg.includes('hello') || lowerMsg.includes('hi') || lowerMsg.startsWith('hey')) {
    return {
  ...
  // Default response
  return {
    content: "I'm here to help with your healthcare management. You can ask me about your documents, medications, appointments, or health insights. How can I assist you further?",
    timestamp: new Date().toISOString(),
    referencedDocuments: []
  };
};
