  ...
  const goToNextStep = () => {
    ...
    if (step === 1) {
      ...
      if (!formData.startTime) {
        setErrors((prev) => ({
          ...prev,
          startTime: 'Please select a time slot',
        }));
        return;
      }
    }
  ...
  return (
    <div className={styles.formContainer}>
      <div className={styles.formSteps}>
        <div
          className={`${styles.formStep} ${step >= 1 ? styles.activeStep : ''} ${step > 1 ? styles.completedStep : ''}`}
        >
          <div className={styles.stepNumber}>{step > 1 ? <FiCheck /> : 1}</div>
          <span className={styles.stepLabel}>Provider</span>
        </div>
        <div className={styles.stepDivider}></div>
        <div
          className={`${styles.formStep} ${step >= 2 ? styles.activeStep : ''} ${step > 2 ? styles.completedStep : ''}`}
        >
          <div className={styles.stepNumber}>{step > 2 ? <FiCheck /> : 2}</div>
          <span className={styles.stepLabel}>Date & Time</span>
        </div>
        <div className={styles.stepDivider}></div>
        <div
          className={`${styles.formStep} ${step === 3 ? styles.activeStep : ''}`}
        >
        ...
        {step === 1 && (
          <div className={styles.formSection}>
            ...
            <div className={styles.searchContainer}>
              ...
              <Link
                to="/appointments/provider-search"
                className={styles.findProvidersButton}
              >
                Find New Providers
              </Link>
            </div>
          ...
          <div className={styles.formSection}>
            ...
            {formData.date && (
              <>
                ...
                />
                {errors.startTime && (
                  <div className={styles.errorText}>{errors.startTime}</div>
                )}
              </>
            )}
