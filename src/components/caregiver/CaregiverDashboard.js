...
const CaregiverDashboard = () => {
  ...
  const totalAlerts = patients?.reduce((sum, p) => sum + (p.alertCount || 0), 0) || 0;
  const upcomingAppointments = patients?.reduce((sum, p) => sum + (p.upcomingAppointmentsCount || 0), 0) || 0;
  ...
  return (
    <div className={styles.caregiverDashboard}>
      ...
      <div className={styles.statsGrid}>
        {stats.map((stat, index) => (
          <Link key={index} to={stat.link} className={styles.statCard}>
            <div className={`${styles.statValue} ${stat.highlight ? styles.highlightValue : ''}`}>
              {stat.value}
            </div>
      ...
      {totalAlerts > 0 && (
        <section className={styles.alertsSection}>
          <div className={styles.sectionTitle}>
            <h2><FiActivity className={styles.sectionIcon} /> Critical Alerts</h2>
      ...
      <section className={styles.patientsSection}>
        <div className={styles.sectionTitle}>
          <h2><FiUsers className={styles.sectionIcon} /> Your Patients</h2>
          {showAddButton && (
        ...
        {loading ? (
          ...
          <div className={styles.emptyState}>
            <p>You don't have any patients assigned yet. Add your first patient to get started!</p>
